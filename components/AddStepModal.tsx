...
const AddStepModal: React.FC<AddNodePanelProps> = ({ onClose, onAddStep }) => {
  ...
  const handleAdd = () => {
    ...
    if (stepType === StepType.PREBUILT && selectedPrebuilt) {
      newStep = {
        type: StepType.PREBUILT,
        name: selectedPrebuilt.name,
        prompt: selectedPrebuilt.prompt,
      };
  ...
  const isAddDisabled =
    (stepType === StepType.PREBUILT && !selectedPrebuilt) ||
    (stepType === StepType.CUSTOM && !customPrompt.trim());
      ...
      <div className="flex-grow space-y-6 overflow-y-auto pr-2 -mr-2">
        <div>
          ...
          <DropDownList
            ...
            onChange={(e) => setStepType(e.target.value.value)}
            className="w-full"
          />
        </div>
        ...
        {stepType === StepType.PREBUILT ? (
          <div className="animate-fade-in">
            <Label
              editorId={prebuiltPromptID}
              className="block text-sm font-medium text-gray-300 mb-2"
            >
              Select a Pre-built Prompt
            </Label>
            <DropDownList
              id={prebuiltPromptID}
          ...
          <div className="animate-fade-in">
            ...
            <TextArea
              id={customPromptID}
              value={customPrompt}
              onChange={(e) => setCustomPrompt(e.target.value as string)}
