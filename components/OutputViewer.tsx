...
const OutputViewer: React.FC<OutputViewerProps> = ({ outputs, isLoading }) => {
  ...
  const downloadAll = () => {
    outputs.forEach((output, index) => {
      setTimeout(() => downloadImage(output.dataUrl, output.originalFileName), index * 300);
    });
  };
    ...
    <div className="p-4 bg-gray-800 border border-gray-700 rounded-lg shadow-lg min-h-[400px] h-full flex flex-col">
      <div className="flex justify-between items-center border-b border-gray-600 pb-2 mb-4 flex-shrink-0">
        <h2 className="text-xl font-bold text-gray-200">
          3. Output {outputs.length > 0 && <span className="text-gray-400 font-normal">({outputs.length} generated)</span>}
        </h2>
        {outputs.length > 1 && (
          <Button themeColor="primary" icon="download" onClick={downloadAll}>Download All</Button>
        )}
      </div>

      <div className="flex-grow overflow-y-auto relative">
        {outputs.length === 0 ? <EmptyState /> : (
          <div className="grid gap-4 p-1 [grid-template-columns:repeat(auto-fill,minmax(200px,1fr))]">
            {outputs.map((output, index) => (
              <div
                ...
                role="button" tabIndex={0} aria-label={`Preview ${output.originalFileName}`}
                onKeyDown={e => { if (e.key === 'Enter' || e.key === ' ') setPreviewImage(output); }}
                style={{ animationDelay: `${index * 60}ms`}}
              >
                <img
                  src={output.dataUrl} alt={`Edited version of ${output.originalFileName}`}
                ...
                <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/80 to-transparent p-2">
                  <p className="text-white text-xs truncate" title={output.originalFileName}>{output.originalFileName}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      {previewImage && (
        <div className="fixed inset-0 bg-gray-900/90 z-50 flex flex-col p-4 sm:p-8 animate-fade-in backdrop-blur-md" onClick={() => setPreviewImage(null)}>
          <div className="w-full max-w-6xl mx-auto flex flex-col h-full" onClick={e => e.stopPropagation()}>
            <div className="flex justify-between items-center mb-4 flex-shrink-0">
              <h3 className="text-lg font-medium text-gray-200 truncate pr-4" title={previewImage.originalFileName}>
                Preview: {previewImage.originalFileName}
              </h3>
              <div className="flex items-center space-x-2 flex-shrink-0">
                <Button themeColor="primary" icon="download" onClick={() => downloadImage(previewImage.dataUrl, previewImage.originalFileName)}>Download</Button>
                <Button icon="close" fillMode="flat" themeColor="light" onClick={() => setPreviewImage(null)} title="Close preview" />
              </div>
            </div>
            <div className="flex-grow flex items-center justify-center min-h-0 bg-black/30 rounded-lg p-2">
              <img src={previewImage.dataUrl} alt={`Preview of ${previewImage.originalFileName}`} className="max-w-full max-h-full object-contain rounded"/>
            </div>
          </div>
        </div>
      )}
