...
const FlowEditor: React.FC<FlowEditorProps> = ({
  ...
  const addNode = (step: Omit<WorkflowNode, "position" | "id">) => {
    ...
    onNodesChange([
      ...nodes,
      {
        ...
        position: {
          x: (canvasBounds?.width || 800) / 2 - 125 + offset,
          y: 50 + offset,
        },
      },
    ]);
  ...
  const handleConfirmRemove = () => {
    ...
    onConnectionsChange(
      connections.filter(
        (conn) =>
          conn.fromNodeId !== nodeToDelete && conn.toNodeId !== nodeToDelete,
      ),
    );
        ...
        nodes.map((n) =>
          n.id === draggingNode.id
            ? {
                ...n,
                position: {
                  x: mousePos.x - draggingNode.offset.x,
                  y: mousePos.y - draggingNode.offset.y,
                },
              }
            : n,
        ),
  ...
  const handleStartConnection = (fromNodeId: string) => {
    ...
    setConnecting({
      fromNodeId,
      toMouse: {
        x: r.x - canvasRect.left + r.width / 2,
        y: r.y - canvasRect.top + r.height / 2,
      },
    });
  };
  ...
  const handleEndConnection = (toNodeId: string) => {
    if (connecting && connecting.fromNodeId !== toNodeId) {
      const alreadyExists = connections.some(
        (c) =>
          c.fromNodeId === connecting.fromNodeId && c.toNodeId === toNodeId,
      );
      if (!alreadyExists) {
        onConnectionsChange([
          ...connections,
  ...
  const connectionPaths = useMemo(() => {
    ...
    return connections
      .map((conn) => {
        ...
        const fromPos = {
          x: fromRect.x - canvasRect.x + fromRect.width / 2,
          y: fromRect.y - canvasRect.y + fromRect.height / 2,
        };
        const toPos = {
          x: toRect.x - canvasRect.x + toRect.width / 2,
          y: toRect.y - canvasRect.y + toRect.height / 2,
        };
        return (
          <path
            key={conn.id}
  ...
  const connectingPath = useMemo(() => {
    ...
    const fromSocket = socketRefs.current.get(
      `${connecting.fromNodeId}-output`,
    );
    const canvasRect = canvasRef.current?.getBoundingClientRect();
    if (!fromSocket || !canvasRect) return null;
    const r = fromSocket.getBoundingClientRect();
    const fromPos = {
      x: r.x - canvasRect.x + r.width / 2,
      y: r.y - canvasRect.y + r.height / 2,
    };
    return (
      <path
        d={getCurvePath(fromPos, connecting.toMouse)}
        stroke="#a5b4fc"
        strokeWidth="2"
        strokeDasharray="6,4"
        fill="none"
    ...
    <div className="p-4 bg-gray-800 border border-gray-700 rounded-lg shadow-lg h-full flex flex-col">
      <div className="flex justify-between items-center border-b border-gray-600 pb-2 mb-4">
        ...
        <div className="flex items-center space-x-2">
          <Button icon="save" onClick={onSave} title="Save Workflow">
            Save
          </Button>
          <Button icon="delete" onClick={onClear} title="Clear Workflow">
            Clear
          </Button>
          {onOpenApiKeyConfig && (
            <Button
          ...
          <Button
            ...
            themeColor="success"
            onClick={onExecute}
            disabled={isExecuting || nodes.length === 0 || !hasImages}
          >
            {isExecuting ? "Executing..." : "Execute Workflow"}
          </Button>
        </div>
      </div>
      <div className="flex-grow relative overflow-hidden rounded-md min-h-[320px] sm:min-h-[480px]">
        <div
          ref={canvasRef}
          ...
          {nodes.length === 0 && (
            <div className="absolute inset-0 flex items-center justify-center pointer-events-none text-center p-4">
              <p className="text-gray-500 max-w-xs">
                Click "Add Node" to begin building your workflow. Drag a node's
                output socket to another's input to connect them.
              </p>
            </div>
          )}
        </div>
      ...
      {nodeToDelete && (
        <Dialog title="Confirm Deletion" onClose={handleCancelRemove}>
          ...
          <DialogActionsBar>
            ...
            <Button onClick={handleConfirmRemove} themeColor={"primary"}>
              Delete
            </Button>
          </DialogActionsBar>
        </Dialog>
