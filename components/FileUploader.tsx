...
const FileUploader: React.FC<FileUploaderProps> = ({ onFilesChange }) => {
  ...
  const processFiles = useCallback(
    (files: FileList) => {
      const newRawFiles = Array.from(files).filter((file) =>
            ...
            <div className="flex-grow overflow-y-auto pr-2 -mr-2 rounded-md">
              <div className="grid grid-cols-3 sm:grid-cols-4 gap-3">
                {imageFiles.map((imageFile) => (
                  <div
                    key={imageFile.id}
                    className="relative group aspect-square rounded-md overflow-hidden animate-fade-in"
                  >
                    <img
                    ...
                    <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                      <Button
                        ...
                        onClick={(e) => {
                          e.stopPropagation();
                          handleRemove(imageFile.id);
                        }}
                        title={`Remove ${imageFile.file.name}`}
                        className="!w-8 !h-8 rounded-full hover:bg-white/10 flex items-center justify-center text-gray-200"
