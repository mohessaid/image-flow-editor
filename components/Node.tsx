...
const Node: React.FC<NodeProps> = ({
  ...
  return (
    ...
    >
      <div
        className="flex items-start justify-between pb-2 border-b border-gray-600 cursor-move"
        onMouseDown={onMouseDown}
      >
        <div className="flex-1 mr-2">
          <h3 className="font-bold text-indigo-400 pointer-events-none text-base leading-tight">
            {node.name}
          </h3>
        ...
        <Button
          ...
          title="Remove node"
          className="!w-6 !h-6 rounded-full flex items-center justify-center text-gray-200 hover:bg-white/10"
      ...
      <div
        ref={(el) => {
          socketRefs.current.set(`${node.id}-input`, el);
        }}
      ...
      <div
        ref={(el) => {
          socketRefs.current.set(`${node.id}-output`, el);
        }}
